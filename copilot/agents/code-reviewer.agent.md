...
4. **Determine review scope** — ask the user: quick review (PR diff only) or deep review (full module analysis)?
5. **Read the diff first**, then open full files for surrounding context on anything suspicious
6. **Trace data flow** through changed functions — follow inputs from entry point to storage and back
7. **Check test coverage** for changed code — are new branches/paths tested?
8. Review systematically against the checklist in `.github/prompts/code-review.prompt.md`
