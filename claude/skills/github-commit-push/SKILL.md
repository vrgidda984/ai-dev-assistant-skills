...
description: >
  Commit changes with logical grouping and push the branch. Analyzes the working
  tree, groups related changes by concern, creates separate Conventional Commits,
  and pushes. Requires user confirmation before every git action.
  Use when the user says "commit and push", "push my changes", "commit everything",
  or invokes /github-commit-push.
---
...
When the user wants to commit and push their current changes with well-structured commit messages.
...
If this skill doesn't auto-trigger, use: `/github-commit-push`
